...
class Auth extends CI_Controller
{
	...
	public function index()
	{
		...
		} else if (!$this->ion_auth->in_group(2)) // remove this elseif if you want to enable this for non-admins
		{
	...
	public function proses_resetpassword()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$cek = $this->ion_auth->username_check($username);
			if ($cek) {
				...
				if ($this->form_validation->run() === FALSE) {
					...
					$user = $this->db->from('users')->where('username', $username)->get()->row_object();
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run() == false) {
				...
				if ($this->ion_auth->register($username, $password, $email, $additional_data, $group)) {
					...
					// $data = array(
					// 	'content' => 'home/formregister',
					// 	'message' => 'Proses register gagal diproses'
					// );
					// $this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
					$data['title'] = 'Register';
					$this->load->view('auth/register', $data);
